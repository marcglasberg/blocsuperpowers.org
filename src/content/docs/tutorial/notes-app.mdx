...
<img src="/notes_app_screenshot1.png" alt="Notes app screenshot"
     style="border: 8px solid #bbb; border-radius: 12px; display: block; margin: 30px auto;"/>
...
_Note the API call in `await api.fetchNotes()` was set up to fail to demonstrate retries.
...
However, the 5th attempt will keep retrying until it finally fails and shows an error. Try it!_
...
_Note that throwing a `StateError` and then catching it to convert to a `UserException`
in a `catchError` block is just to demonstrate the feature.
We could have directly thrown a `UserException` instead:
`if (text.trim().isEmpty) throw UserException("Note can't be empty.");`_

<img src="/notes_app_screenshot2.png" alt="Notes app screenshot"
     style="border: 8px solid #bbb; border-radius: 12px; display: block; margin: 30px auto;"/>
...
<img src="/notes_app_screenshot3.png" alt="Notes app screenshot"
     style="border: 8px solid #bbb; border-radius: 12px; display: block; margin: 30px auto;"/>
